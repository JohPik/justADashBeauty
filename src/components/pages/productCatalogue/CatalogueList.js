...
class CatalogueList extends Component {
  ...
  componentWillMount() {
    ...
    return skinId === skinType && prodId === prodType
      ? null
  ...
  resetFilter = (type) => {
    ...
    if (type === 'skinType') {
      ...
      this.props.history.push(
        `/shop/skintype=${skinType}&prodtype=${prodType}`
      );
    } else {
  ...
  renderList = (value) => {
    if (this.state.skinType === 'all' && this.state.prodType === 'all') {
      return value.productList.map((prod) => (
        <ThumbnailProd prod={prod} key={prod.id} />
      ));
    } else if (this.state.prodType === 'all') {
      return value.productList
        .filter((prod) => prod.skinType.includes(this.state.skinType))
        .map((prod) => <ThumbnailProd prod={prod} key={prod.id} />);
    } else if (this.state.skinType === 'all') {
      return value.productList
        .filter((prod) => prod.productType.includes(this.state.prodType))
        .map((prod) => <ThumbnailProd prod={prod} key={prod.id} />);
    } else {
      const nbrOfProd = value.productList.filter(
        (prod) =>
          prod.skinType.includes(this.state.skinType) &&
          prod.productType.includes(this.state.prodType)
      );
      return nbrOfProd.length === 0 ? (
        <p>Sorry no product match you research</p>
      ) : (
        nbrOfProd.map((prod) => <ThumbnailProd prod={prod} key={prod.id} />)
      );
    }
  };
